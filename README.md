...
## 🗄️ Neo4j Graph Database Setup & Usage

> **Note**: Neo4j is **OPTIONAL**. All FOEP functionality works without it. Neo4j only enables graph visualization and complex relationship queries.

### Option 1: Docker Setup (Recommended for Kali/Ubuntu)
...
### Option 2: Ubuntu/Debian Native Install

```bash
# Add Neo4j repository
curl https://debian.neo4j.com/neotechnology.gpg.key | sudo apt-key add -
sudo echo 'deb https://debian.neo4j.com stable latest' | tee /etc/apt/sources.list.d/neo4j.list
...
neo4j:
  ...
  uri: "bolt://localhost:7687"
  username: "neo4j"
  password: "foep-password"  # Or your password
```

### Query Neo4j Browser

After running `foep-correlate`, query the graph:

```cypher
# View all evidence nodes
MATCH (e:Evidence) RETURN e LIMIT 20;

# View entity relationships
MATCH (e:Evidence)-[:RELATED_TO]-(e2:Evidence) 
RETURN e, e2 LIMIT 10;

# Find threats
MATCH (e:Evidence)-[:CONTAINS_THREAT]-(t:Threat) 
RETURN e, t;

# Complex correlation
MATCH (e1:Evidence)-[:RELATED_TO*2]-(e2:Evidence)
WHERE e1.entity_type = 'domain' AND e2.entity_type = 'ip'
RETURN e1, e2;
```
...
| Issue | Solution |
|-------|----------|
| **Connection refused** | Check if running: `docker ps` or `sudo systemctl status neo4j` |
| **Port 7687 already in use** | `sudo lsof -i :7687` to find process, then stop it |
| **Auth failed** | Reset password: `sudo neo4j-admin dbms set-initial-password` |
| **Browser won't load** | Wait 30 seconds after start, check network: `curl http://localhost:7474` |
| **Out of memory** | Increase heap: Edit `neo4j.conf`, set `dbms.memory.heap.max_size=2G` |

---

## 🚨 Threat Detection-Specific Tips

### Only Specific Entity Types Trigger Threat Detection

✅ **CHECKED by Threat Detectors:**
- `entity_type: "hash"` - Malware hash detection (MD5, SHA256)
- `entity_type: "ip"` - Malicious IP detection (Shodan, AbuseIPDB)
- `entity_type: "domain"` - C2 domain detection
- `entity_type: "url"` - Malicious URL detection
- `entity_type: "file"` - File behavior analysis
- `entity_type: "command_line"` - LOLBin detection

❌ **NOT CHECKED (OSINT-only):**
- `entity_type: "username"` - GitHub/Twitter users are not threat-checked
- `entity_type: "email"` - HIBP results are not threat-checked
- `entity_type: "person_name"` - Names are not threat-checked

### To Trigger Threat Detection

**Option 1: Use Malware Hashes**
```bash
foep-ingest \
  --vt-hash "5d41402abc4b2a76b9719d911017c592" \
  --output malware_evidence.json
# Look for threat_intel in metadata if hash is known malware
```

**Option 2: Use Suspicious Infrastructure**
```bash
foep-ingest \
  --domain "command-and-control.com" \
  --output infrastructure.json
# Shodan may identify exposed services
```

**Option 3: Use Forensic Analysis**
```bash
foep-ingest \
  --disk /path/to/disk.E01 \
  --output forensic.json
# File hashes extracted will be checked
```

**Option 4: Analyze with Shodan**
```bash
foep-ingest \
  --shodan "192.168.1.0/24" \
  --output network_scan.json
# IPs will be checked against threat intelligence
```

### Why No Threats in OSINT Results?

**This is NORMAL.** GitHub users, Twitter accounts, and public domains are not malicious by default. Threat detection only flags:
- Known malware hashes
- Botnet C2 IPs
- Phishing domains
- Suspicious infrastructure

Public OSINT results (legitimate usernames, public domains) will NOT have threat data.

---

## 📁 Generated JSON Files Reference

### evidence.json
**Purpose:** Raw ingested evidence  
**Contents:** 10-50 items from OSINT sources
**Format:**
```json
[
  {
...
### correlated.json
**Purpose:** Evidence after entity linking  
**Format:**
```json
[
  {
    "evidence_id": "dns::domain_example.com",
    "entity_type": "domain",
    "metadata": {
      "linkage_group_id": "group_0",
      "linked_entities": ["ip_192.168.1.100"],
      "credibility_adjustments": { "corroboration_bonus": 5 }
    }
  },
  ...
]
...
```json
{
  ...
  "evidence_items": [ /* All evidence */ ]
}
```

### threat_intelligence.json (Demo Output)
**Purpose:** Parsed threat intelligence from 3 sources  
**Contains:**  
- AbuseIPDB results for IPs
- OTX results for domains/hashes
- Shodan infrastructure data

### verdict_report.json (Demo Output)
**Purpose:** Automated forensic verdict  
**Contains:**
- Verdict: MALICIOUS / SUSPICIOUS / BENIGN
- Confidence level: 0-100%
- Threat evidences identified
- Actionable recommendations
...
|-------|---------|----------|
| **No threat_intel in JSON** | Evidence lacks `metadata.threat_intel` key | ✅ NORMAL for OSINT-only. See "Threat Detection-Specific Tips" above |
| **API Rate Limited** | `429 Too Many Requests` error | Add API keys to `config/config.yaml` or wait between requests |
| **Neo4j Connection Fails** | `Connection refused: 127.0.0.1:7687` | ✅ NOT FATAL. Start Docker or native Neo4j per section above |
| **GitHub API Error** | `GitHub API error or timeout` | Check GitHub token in config, verify internet connection |
| **VirusTotal Not Working** | No VT results in output | Enable and add API key to `config/config.yaml` |
| **Disk Image Not Found** | `FileNotFoundError` for disk path | Verify disk image path is correct and readable |
| **Module Import Error** | `ModuleNotFoundError: foep` | Run `pip install -e .` from project root |
...
│                                                     │
│  SOURCE COLLECTION                                  │
│  ├─ Forensic: Disk → Files, Hashes, Metadata      │
│  ├─ Forensic: Memory → Running processes            │
│  ├─ Forensic: Logs → Events, Timestamps             │
│  ├─ OSINT: GitHub → Users, Repos, Activity          │
│  ├─ OSINT: Twitter → Posts, Followers               │
│  ├─ OSINT: Domain → NS, MX, DNS records             │
│  ├─ OSINT: HIBP → Breach records                    │
│  ├─ OSINT: VirusTotal → Hash reputation             │
│  └─ OSINT: Shodan → Infrastructure exposure         │
│                     ↓                                │
│          evidence.json (Raw)                         │
│         [10-50 items collected]                     │
│                     ↓                                │
│  THREAT DETECTION (Optional)                        │
│  ├─ HashDetector: Check VM/Shodan for file hashes   │
│  ├─ IPDetector: AbuseIPDB, Shodan for IPs           │
│  ├─ DomainDetector: OTX, Shodan for domains         │
│  ├─ URLDetector: VirusTotal for URLs                │
│  └─ (Usernames/Emails: No threat check)             │
│                     ↓                                │
│  If threats found → metadata.threat_intel populated  │
│  If no threats → No threat_intel key (normal)       │
│                     ↓                                │
│  ENTITY LINKING & CREDIBILITY                       │
│  ├─ Extract entities (IP, domain, email, etc.)      │
│  ├─ Link related entities (e.g., IP → Domain)       │
│  ├─ Calculate corroboration scores                  │
│  └─ Adjust credibility (+5 for corroboration)       │
│                     ↓                                │
│         correlated.json (Enhanced)                   │
│       [Same items + linkage + scoring]              │
│                     ↓                                │
│  OPTIONAL: Neo4j Graph Creation                     │
│  ├─ Create :Evidence nodes                          │
│  ├─ Create :Incident nodes (if threats)             │
│  └─ Create relationships (-[:RELATED_TO]->)         │
│                     ↓                                │
│  REPORT GENERATION                                  │
│  ├─ HTML Report with chain of custody               │
│  ├─ Custody JSON with evidence metadata             │
│  ├─ Threat summary (if threats detected)            │
│  └─ Redacted version (PII removed)                  │
│                     ↓                                │
│    reports/foep_report_*.html (Court-ready)         │
│    reports/custody_*.json (Digital chain)           │
│    reports/threat_summary.json (Analysis)           │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
- **For large disk images**: Use `--disk /path/image.E01 --output out.json --preserve=recent` to limit recent file analysis
- **For batch processing**: Run multiple `foep-ingest` commands in parallel with different `--case-id` values
- **For Neo4j**: Increase heap size in `neo4j.conf` if handling 1000+ evidence items
- **For API rates**: Configure API keys in `config/config.yaml` to increase quotas
...
- Shodan.io for infrastructure data
